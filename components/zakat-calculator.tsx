...
export function ZakatCalculator() {
  ...
  const calculateZakat = () => {
    ...
    const totalWealth = cashAmount + goldAmount + silverAmount + investmentAmount + businessAmount - debtAmount;
              ...
              <Tabs defaultValue="assets" className="w-full">
                ...
                <TabsContent value="debts" className="space-y-4">
                  ...
                  <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <h4 className="font-semibold text-blue-900 mb-2">Nisab Threshold</h4>
                    <p className="text-sm text-blue-800">
                      Current Nisab (minimum amount): ~${Math.min(
                        GOLD_NISAB * GOLD_PRICE_PER_GRAM,
                        SILVER_NISAB * SILVER_PRICE_PER_GRAM
                      ).toLocaleString()}
                    </p>
                    <p className="text-xs text-blue-700 mt-1">
                      Based on current gold/silver prices. Your wealth must exceed this amount for one lunar year.
                    </p>
                  </div>
                </TabsContent>
              </Tabs>
              ...
              {showResults ? (
                <div className="space-y-6">
                  ...
                  <div className="text-center p-6 bg-islamic-gradient text-white rounded-lg">
                    ...
                    <p className="text-4xl font-bold">${totalZakat.toLocaleString()}</p>
                    <p className="text-islamic-100 mt-2">
                      {totalZakat > 0 ? 'Zakat is obligatory' : 'No Zakat due (below Nisab threshold)'}
                    </p>
                  </div>
                          ...
                          <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                            <div>
                              ...
                              <div className="text-sm text-gray-600">
                                ${result.amount.toLocaleString()} × {result.rate}%
                              </div>
                            </div>
                  ...
                  <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                    <p className="text-sm text-green-800">
                      <strong>Important:</strong> Zakat should be paid to eligible recipients as specified in the Quran. 
                      This includes the poor, needy, those in debt, and other categories mentioned in Islamic law.
                    </p>
                  </div>

                  <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p className="text-sm text-yellow-800">
                      <strong>Note:</strong> This calculator provides estimates based on standard rates. 
                      Consult with Islamic scholars for complex situations or specific religious guidance.
                    </p>
                  </div>
                </div>
              ) : (
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p>Enter your wealth information and calculate to see Zakat due</p>
                </div>
              )}
            </CardContent>
          </Card>
